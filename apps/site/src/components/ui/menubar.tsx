...
// - Overlay shadow tokens and project typography.
...
function MenubarTrigger({
  ...
  return (
    <MenubarPrimitive.Trigger
      ...
      className={cn(
        `flex cursor-pointer items-center rounded-sm px-1.5 py-[2px] text-sm ${FONT_WEIGHT_NORMAL} outline-hidden select-none hover:bg-muted aria-expanded:bg-muted`,
        className,
      )}
      {...props}
    />
...
function MenubarLabel({
  ...
  return (
    <MenubarPrimitive.Label
      ...
      data-inset={inset}
      className={cn(`px-1.5 py-1 text-sm ${FONT_WEIGHT_NORMAL} data-inset:pl-7`, className)}
      {...props}
    />
  );
}
