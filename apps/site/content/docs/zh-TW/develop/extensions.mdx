...
這與 [Skills](../customize/skills.mdx) (`SKILL.md` 資料夾)、[MCP](../customize/mcp.mdx) (外部伺服器) 和 [Hooks](../customize/hooks.mdx) (`hooks.json` 腳本) 不同。`contributes.cli.hooks` 樣式化 CLI TUI 插槽。它不是 `hooks.json`。擴充功能仍可在宣告 `mcp`、`hooks`、`skills` 或 `rules` 後，用相同檔案規範捆綁這些資源。

每個已安裝的擴充功能都有啟用開關。停用或解除安裝後即停止貢獻。開啟開關即同意執行它宣告的 MCP、agent hooks、skills 與 rules。
...
| `contributes`           | 選用。`tools`、`desktop`、`cli`、`mcp`、`hooks`、`skills` 和/或 `rules` |
