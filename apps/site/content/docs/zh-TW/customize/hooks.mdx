...
在 **設定 → Hooks** 中管理它們。使用 CLI 的 `spirit hooks list` 和 `spirit hooks validate` 進行驗證。內建的 Skill `create-hook` 會草擬一個 hook。[擴充功能](../develop/extensions.mdx) 捆綁的 hooks 只在該擴充功能啟用時執行，不會出現在此設定清單。
