...
Cela diffère des [Skills](../customize/skills.mdx) (dossiers `SKILL.md`), des [MCP](../customize/mcp.mdx) (serveurs externes) et des [Hooks](../customize/hooks.mdx) (scripts `hooks.json`). `contributes.cli.hooks` stylise les emplacements CLI TUI. Ce n'est pas `hooks.json`. Une extension peut bundler ces fichiers si elle déclare `mcp`, `hooks`, `skills` ou `rules`.

Chaque extension installée a un interrupteur d'activation. La désactiver ou la désinstaller arrête ses contributions. L'activer constitue le consentement à exécuter les MCP, hooks agent, skills et rules déclarés.
...
| `contributes`           | Optionnel. `tools`, `desktop`, `cli`, `mcp`, `hooks`, `skills` et/ou `rules` |
