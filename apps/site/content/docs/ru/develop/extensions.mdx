...
Это отличается от [Skills](../customize/skills.mdx) (папки `SKILL.md`), [MCP](../customize/mcp.mdx) (внешние серверы) и [Hooks](../customize/hooks.mdx) (скрипты `hooks.json`). `contributes.cli.hooks` оформляет слоты CLI TUI. Это не `hooks.json`. Расширение может поставлять те же файлы, если объявит `mcp`, `hooks`, `skills` или `rules`.

У каждого установленного расширения есть переключатель включения. Отключение или удаление останавливает вклад. Включение — это согласие запускать объявленные MCP, agent hooks, skills и rules.
...
| `contributes`           | Необязательное. `tools`, `desktop`, `cli`, `mcp`, `hooks`, `skills` и/или `rules` |
