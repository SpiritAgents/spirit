...
Manage them in **Settings → Hooks**. Validate from the CLI with `spirit hooks list` and `spirit hooks validate`. The built-in Skill `create-hook` drafts a hook. Hooks bundled in an [extension](../develop/extensions.mdx) run when that extension is enabled. They do not appear in this settings list.
